import urllib.request, urllib.error, urllib.parse, json, csv
from time import localtime, strftime

record_time = strftime("%a, %d %b %Y %H:%M:%S", localtime())
name_time = strftime("%Y%m%d_%H%M%S", localtime())

response = urllib.request.urlopen('https://www.imperialcollegeunion.org/mumsanddads/stats.php')
html = response.read()
...
with open(name_time+'_stats.csv', 'w') as f:
    writer = csv.writer(f)
